...
function ModalUpdateUser(props) {
    ...
    useEffect(() => {
        if (_.isEmpty(userClick)) {
        }
        else {
            setUsername(userClick.username)
            setRole(userClick.role);
            setImage(`data:image/png;base64, ${userClick.image}`);
    ...
    const handleSubmit = async () => {
        //submit form
        const data = await putUpdateUser(userClick.id, username, role, image)
        if (data && data.EC === 0) {
                    ...
                    <form className='modalCreateUser__form'>
                        <div className="row">
                            ...
                            <div className="form-group col-md-6">
                                ...
                                <input disabled type="password" className="form-control" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} />
                            </div>
                        </div>
